"""
Base Validation - Core validation logic for external labels
...
This base class is inherited by specialized validation classes:
- GICSValidation (GICS sector-based validation)
- SizeValidation (company size-based validation)
"""
...
class BaseValidation:
    """
    Base class for validating clustering results against external categorical labels
    ...
    def __init__(self, validation_type: str = "generic"):
        """
        Initialize Base Validation

        Args:
            validation_type: Type of validation (for logging purposes)
        """
        self.validation_type = validation_type
        logger.info(f"✓ {validation_type.title()}Validation initialized")
    ...
    def generate_validation_report(
        ...
        """
        logger.info("\n" + "=" * 80)
        logger.info(f"🔬 {self.validation_type.upper()} VALIDATION REPORT")
        logger.info("=" * 80)
